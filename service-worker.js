...
const CACHE_NAME = 'nihongo-n5-v3';
...
self.addEventListener('install', event => {
  ...
  event.waitUntil(
    caches.open(STATIC_CACHE).then(cache => {
      // Cache what we can, but don't fail install if fonts are unreachable
      return Promise.allSettled(
        PRECACHE_FILES.map(url => cache.add(url).catch(e => console.warn('[SW] Failed to cache:', url, e)))
      );
    })
  );
  self.skipWaiting();
